...
export default function LeaguePage() {
  ...
  useEffect(() => {
    ...
    getLockStatus()
      .then((s) => {
        setIsLocked(s.locked);
        if (s.active_gameweek) {
          setSelectedWeek(s.active_gameweek);
        }
      })
      .catch(() => {});
  }, [router]);
      ...
      <div className="pt-24 pb-12 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {tab === "view" && leagueDetail && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            {/* Player Performances */}
            {viewSubTab === "gameweek" && !gameweekLoading && weekPlayers.length > 0 && (
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full font-bold" style={{ background: "rgba(0, 230, 118, 0.1)", color: "var(--accent-green)" }}>
                      {weekPlayers.length}
                    </span>
                  </div>
                  <motion.div
                ...
                <AnimatePresence>
                  {showPlayerStats && (
                    ...
                    >
                      {/* Table header */}
                      <div
                        className="grid items-center px-4 py-2 text-[10px] uppercase tracking-wider font-bold"
                        style={{
                        ...
                        {weekPlayers.map((player, i) => {
                          ...
                          return (
                            <div
                              ...
                              style={{
                                gridTemplateColumns: "1fr repeat(5, 40px)",
                                background: isTopPoints ? "rgba(255, 171, 0, 0.04)" : "transparent",
                                borderBottom: "1px solid var(--border-color)",
                              }}
                            >
                              <div className="flex items-center gap-2 min-w-0">
                                ...
                                {isTopPoints && (
                                  <span className="text-[9px] font-bold px-1 py-0.5 rounded shrink-0" style={{ background: "rgba(255,171,0,0.2)", color: "#ffab00" }}>
                                    MVP
                                  </span>
                                )}
                              </div>
                              <span className="text-xs text-center font-bold" style={{ color: player.goals > 0 ? "var(--accent-green)" : "var(--text-muted)" }}>
                                {player.goals || "-"}
                              </span>
                              <span className="text-xs text-center font-bold" style={{ color: player.assists > 0 ? "var(--accent-amber)" : "var(--text-muted)" }}>
                                {player.assists || "-"}
                              </span>
                              <span className="text-xs text-center font-bold" style={{ color: player.clean_sheets > 0 ? "#42a5f5" : "var(--text-muted)" }}>
                                {player.clean_sheets || "-"}
                              </span>
                              <span className="text-xs text-center" style={{ color: "var(--text-muted)" }}>
                                {player.minutes_played || "-"}
                              </span>
                              <span className="text-xs text-center font-bold" style={{ fontFamily: "var(--font-display)", color: "var(--accent-green)" }}>
                                {player.total_points}
                              </span>
                            </div>
                          );
